...
impl PublishDiagnosticsParams {
    pub fn from(
        ...
        let diagnostics = Self::map_cargo_diagnostic_to_bsp(msg, workspace_root);
        diagnostics
            .into_iter()
    ...
    fn map_cargo_diagnostic_to_bsp(
        ...
        let severity = Self::diagnostic_severity(diagnostic.level);
        ...
        if let Some(code_val) = &code {
            ...
            if scoped_code.len() == 2 {
                if scoped_code[0].eq("clippy") {
                    code_description = Self::clippy_code_description(code.as_deref());
                }
                code = Some(String::from(scoped_code[1]));
            } else {
                code_description = Self::rustc_code_description(code.as_deref());
            }
        }

        let tags = Self::diagnostic_tags(&diagnostic.code);
        ...
        for secondary_span in diagnostic.spans.iter().filter(|s| !s.is_primary) {
            let related = Self::diagnostic_related_information(workspace_root, secondary_span);
        ...
        for child in &diagnostic.children {
            let child = Self::map_rust_child_diagnostic(workspace_root, child);
            match child {
        ...
        for primary_span in &primary_spans {
            let primary_location = Self::primary_location(workspace_root, primary_span);
            ...
            for (i, span) in span_stack.enumerate() {
                if Self::is_dummy_macro_file(&span.file_name) {
                    continue;
                }
                ...
                let secondary_location = Self::location(workspace_root, span);
                ...
                Self::add_diagnostic(secondary_location.uri, diagnostic, &mut diagnostics);
            }
            ...
            Self::add_diagnostic(primary_location.uri.clone(), diagnostic, &mut diagnostics);
            ...
            for sub in &subdiagnostics {
                ...
                Self::add_diagnostic(sub.location.uri.clone(), diagnostic, &mut diagnostics);
            }
        }
        diagnostics
    }
    ...
    fn location(workspace_root: &AbsPath, span: &DiagnosticSpan) -> lsp_types::Location {
        ...
        let uri = Self::url_from_abs_path(&file_name);

        let range = {
            lsp_types::Range::new(
                Self::position(span, span.line_start, span.column_start),
                Self::position(span, span.line_end, span.column_end),
            )
        };
    ...
    fn diagnostic_related_information(
        ...
        let location = Self::location(workspace_root, span);
    ...
    fn map_rust_child_diagnostic(
        ...
        MappedRustChildDiagnostic::RelatedDiagnostic(lsp_types::DiagnosticRelatedInformation {
            location: Self::location(workspace_root, spans[0]),
            message,
        })
    }
    ...
    fn primary_location(workspace_root: &AbsPath, span: &DiagnosticSpan) -> lsp_types::Location {
        let span_stack =
            std::iter::successors(Some(span), |span| Some(&span.expansion.as_ref()?.span));
        for span in span_stack.clone() {
            let abs_path = workspace_root.join(&span.file_name);
            if !Self::is_dummy_macro_file(&span.file_name) && abs_path.starts_with(workspace_root) {
                return Self::location(workspace_root, span);
            }
        }

        // Fall back to the outermost macro invocation if no suitable span comes up.
        let last_span = span_stack.last().unwrap();
        Self::location(workspace_root, last_span)
    }
