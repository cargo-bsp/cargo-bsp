...
pub(crate) fn handle_disable_cargo_features(
    ...
    params: bsp_types::requests::cargo_extension::DisableCargoFeaturesParams,
) -> Result<()> {
...
pub(crate) fn handle_enable_cargo_features(
    state: &mut GlobalState,
    params: bsp_types::requests::cargo_extension::EnableCargoFeaturesParams,
) -> Result<()> {
    let mutable_workspace = Arc::make_mut(&mut state.workspace);
...
pub(crate) fn handle_cargo_features_state(
    state: GlobalStateSnapshot,
    _: (),
) -> Result<bsp_types::requests::cargo_extension::CargoFeaturesStateResult> {
    let packages_features = state.workspace.get_cargo_features_state();

    Ok(bsp_types::requests::cargo_extension::CargoFeaturesStateResult { packages_features })
}
